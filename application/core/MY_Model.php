...
class MY_Model extends CI_Model
{
    ...
    protected $before_erase = array();
    protected $after_erase  = array();
    ...
    /**
     * Optionally set POST processing against DB fields.
     */
    protected $post_processing = TRUE;
    ...
    public function __construct()
    {
        ...
        array_unshift($this->before_save, 'protect_attributes');
...
*/
    ...
    public function query()
    {
        ...
        if (count($args) == 1)
        {
            $res = $this->db->query($args[0]);
        }
      
        else
        {
            $res = $this->db->query($args[0], $args[1]);
        }
     
        return $res->{$this->_return_type(1)}();
    }
    ...
    public function save()
    {
        ...
        if ($data !== FALSE)
        {
            $data = $this->_process_post($data);
            $data = $this->trigger('before_save', $data);
            
            if($ins AND ( empty($this->db->ar_where) AND empty($this->db->ar_cache_where)))
            {
            ...
            $this->trigger('after_save', $result);
            return $result;
        }
        else
        {
    ...
    {        
        ...
        $where = $this->trigger('before_erase', $args);
        ...
        $this->trigger('after_erase', $result);        
        return $result;
    }
    ...
    /*
    * Process POST data againt DB fields
    * 
    */
    
    private function _process_post($data = NULL)
    {
        if ( ! $this->post_processing) return $data;
        ...
        foreach ($fields as $field)
        {
             ...
             {
                 $obj[$field] = ('' !== $val = trim($this->input->post($field))) ? $val : NULL;
             }
             else
             {
                 if( isset($data[$field]))
                     $obj[$field] = ('' !== $val = trim($data[$field])) ? $val : NULL;   
             }
        }
